...
int main(int argc, char *argv[]) {
  ...
  web_root_dirfd = open(web_root, O_DIRECTORY|O_RDONLY|O_CLOEXEC);
  if (web_root_dirfd < 0)
    die("Failed to open web root directory: %m\n");

  struct open_how safe_open_how = { .flags = O_RDONLY|O_CLOEXEC, .resolve = RESOLVE_IN_ROOT };
  file_fd = openat2(web_root_dirfd, argv[1], &safe_open_how, sizeof(struct open_how));
  if (file_fd < 0)
    die("Failed to open %s: %m\n", argv[1]);

  r = cat_file_fd(file_fd);
  if (r < 0)
